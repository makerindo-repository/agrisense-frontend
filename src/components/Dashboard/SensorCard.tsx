  ...
  return (
    ...
    >
      ...
      >
        ...
        <Card 
          className={cn(
            "absolute inset-0 border-none shadow-sm shadow-black/5 overflow-hidden bg-card flex flex-col transition-all duration-300",
            !isFlipped && "hover:shadow-md hover:-translate-y-1"
          )}
          style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden' }}
        >
          <CardContent className="p-4 flex flex-col h-full z-10 relative">
          ...
          {/* Background Chart */}
          <div className="absolute bottom-0 left-0 right-0 h-12 w-full opacity-40 group-hover:opacity-100 transition-opacity pointer-events-none">
            {hasData && (
        ...
        >
          <CardContent className="p-4 h-full flex flex-col justify-center items-center text-center relative z-10 border border-primary/10 rounded-xl">
            ...
            <p className="text-[9px] text-muted-foreground leading-tight line-clamp-3">
              {description || `Parameter pembacaan ${title}.`}
            </p>
